...
#define STATUS_POWER  0x80u      /* status bit 7: 1 = power was lost  */
...
bool gba_rtc_get_status(GbaRtcStatus* out) {
  GPIO_CTRL = 1;   /* make the GPIO pins readable/writable (as gba_rtc_get does) */

  uint8_t status;
  rtc_txn(CMD_STATUS_READ, &status, 1);

  /* No separate detect call: a flashcart that doesn't drive the GPIO leaves the
   * data line pulled high, so the read comes back 0xFF — reject that. (0x00 is a
   * legitimate idle status: 12-hour mode, no power loss, no interrupts enabled,
   * so it is accepted; the real presence test is gba_rtc_get's range check.) */
  if (status == 0xFFu) return false;

  out->raw        = status;
  out->power_lost = (status & STATUS_POWER) != 0;
  out->mode_24h   = (status & STATUS_24HOUR) != 0;
  return true;
}
