...
// Reboot into the EZ-Flash kernel: map the bootloader page (kernel at
// 0x08000000) then BIOS SoftReset, which clears RAM/regs and jumps to
// 0x08000000 = the kernel. Must be EWRAM-resident: the ROM window is unmapped
// once the rompage is the bootloader. Does not return.
//
// Before the reset we tear down IRQ/DMA/timer state (REG_IME=0 only masks the
// CPU — it does not stop an in-flight DMA or a timer, and there is a window
// after SetRompage(BOOTLOADER) where the ROM/gamepak window is unmapped). This
// mirrors the EverDrive ed_reboot path so both carts reset from a quiesced state.
